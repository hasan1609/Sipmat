package com.sipmat.sipmat.admin.edgblok2.hasil
...
class HasilEdgblok2Activity : AppCompatActivity(), AnkoLogger {
    private lateinit var mAdapter: HasilEdgblok2Adapter
    lateinit var progressDialog: ProgressDialog
    var api = ApiClient.instance()
    var triwulan: String? = null
    lateinit var binding: ActivityHasilEdgblok2Binding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_hasil_edgblok2)
                        ...
                        if (response.isSuccessful) {
                            ...
                            if (data!!.data!!.isEmpty()) {
                                ...
                                for (hasil in data.data!!) {
                                    notesList.add(hasil)
                                    mAdapter =
                                        HasilEdgblok2Adapter(notesList, this@HasilEdgblok2Activity)
                                    binding.rvhasilffblok.adapter = mAdapter
                                    mAdapter.setDialog(object : HasilEdgblok2Adapter.Dialog {

                                        override fun onClick(position: Int, note: EdgBlok2Model) {
                                            val builder =
                                                AlertDialog.Builder(this@HasilEdgblok2Activity)
                                            builder.setMessage("Cek Edg Blok 2 ? ")
                                            builder.setPositiveButton("Cek Edg Blok 2") { dialog, which ->
                                                val gson = Gson()
                                                val noteJson = gson.toJson(note)
                                                startActivity<CekEdgblok2AdminActivity>("cekedgblok2" to noteJson)
                                            }
