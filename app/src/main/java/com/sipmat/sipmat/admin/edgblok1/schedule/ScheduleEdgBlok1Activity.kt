...
class ScheduleEdgBlok1Activity : AppCompatActivity(), AnkoLogger {
    ...
    private lateinit var mAdapter: ScheduleEdgblok1Adapter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_schedule_edgblok1)
        binding.lifecycleOwner = this
        progressDialog = ProgressDialog(this)

        binding.btntambah.setOnClickListener {
            startActivity<TambahScheduleEdgblok1Activity>()
        }
                    ...
                    try {
                        if (response.isSuccessful) {
                            ...
                            for (hasil in data!!.data!!) {
                                notesList.add(hasil)
                                mAdapter = ScheduleEdgblok1Adapter(notesList)
                                binding.rvscheduleffblok.adapter = mAdapter
                                mAdapter.setDialog(object : ScheduleEdgblok1Adapter.Dialog {
                                    override fun onClick(position: Int, note: EdgBlokModel) {

                                        val builder = AlertDialog.Builder(this@ScheduleEdgBlok1Activity)
                                        builder.setMessage("Hapus Schedule ? ")
                                        builder.setPositiveButton(android.R.string.yes) { dialog, which ->
                                            loading(true)
                                            api.hapus_edgblok1(note.id!!).enqueue(object :
