...
import com.sipmat.sipmat.databinding.ActivityScheduleEdgblok3Binding
...
class ScheduleEdgBlok3Activity : AppCompatActivity(), AnkoLogger {
    ...
    private lateinit var mAdapter: ScheduleEdgblok3Adapter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_schedule_edgblok3)
        binding.lifecycleOwner = this
        progressDialog = ProgressDialog(this)

        binding.btntambah.setOnClickListener {
            startActivity<TambahScheduleEdgblok3Activity>()
        }
                    ...
                    try {
                        if (response.isSuccessful) {
                            ...
                            for (hasil in data!!.data!!) {
                                notesList.add(hasil)
                                mAdapter = ScheduleEdgblok3Adapter(notesList)
                                binding.rvscheduleffblok.adapter = mAdapter
                                mAdapter.setDialog(object : ScheduleEdgblok3Adapter.Dialog {
                                    override fun onClick(position: Int, note: EdgBlok3Model) {

                                        val builder = AlertDialog.Builder(this@ScheduleEdgBlok3Activity)
                                                ...
                                                override fun onResponse(
                                                    ...
                                                    try {
                                                        if (response.body()!!.sukses == 1) {
                                                            loading(false)
                                                            toast("hapus Edg blok 3 berhasil")
