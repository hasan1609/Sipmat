...
class TambahScheduleApatActivity : AppCompatActivity(),AnkoLogger {
    ...
    lateinit var apatAdapter : ApatAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btntambah.setOnClickListener {
            sb = StringBuilder()
            var i = 0
            while (i < apatAdapter.lista.size) {
                val spiritualTeacher = apatAdapter.lista[i]
                sb!!.append(spiritualTeacher.kode)
                if (i != apatAdapter.lista.size - 1) {
                    sb!!.append("n")
                }
                i++
            ...
            if (tw.isNotEmpty() && tanggal_cek!=null && apatAdapter.lista.size > 0 && tahun.isNotEmpty()){
                loading(true)
                api.schedule_apat(sb.toString(),tw,tahun,tanggal_cek!!).enqueue(object :Callback<PostDataResponse>{
    ...
    private fun pilihapat() {
        binding.rcCbApat.setHasFixedSize(true)
        binding.rcCbApat.layoutManager = LinearLayoutManager(this)
        (binding.rcCbApat.layoutManager as LinearLayoutManager).orientation =
            LinearLayoutManager.VERTICAL
        api.getapat().enqueue(object : Callback<ApatResponse> {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    for (hasil in data!!.data!!) {
                        notesList.add(hasil)
                        apatAdapter = ApatAdapter(notesList, this@TambahScheduleApatActivity)
                        binding.rcCbApat.adapter = apatAdapter
                        apatAdapter.notifyDataSetChanged()
                    }
                }
            }
        }
